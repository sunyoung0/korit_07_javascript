...
/*
  이상의 부분에서 주의할 점은 한계값 바로 앞까지만 불러온다는 점입니다. 그래서 32번 라인의 경우 12번 index까지라고 했을 때 Banana라는 결과값이라고 착각할 수 있습니다.

  즉, slice()의 두 번째 매개변수를 for문에서의 한계값 설정할 때 제가 이상/이하보다는 초과/미만을 사용하는 경우가 많은데, 그 이유가 여기서도 적용됩니다. 
*/
...
/* 
  ...
  마이너스 인덱스
  맨마지막 문자열의 index를 -1으로 잡고 앞으로 올때마다 1씩 더해줍니다.
*/
...
/*
  ...
  7번지로 출력하려고 하면 12일5678의 경우 index number 7이 없기 때문에 오류 발생
  -1쓰면 다 해결됨
*/
...
/*
  86번 라인의 경우 처음보는 형태
  JS 정규식 표현식을 썼는데 추후 설명 예정이고
  i는 insensitiv의 축약어로 덜 깐깐하게 굴겠다는겁니다.
  대소문자 구분 안하고 그냥 찾는다는 의미
*/
let rpdStc3 = eStc.replace(/Seoul/g, 'Busan');
console.log(rpdStc3);
// 결과값 : Please visit Busan and Busan
// /g 는 glodal 약자고, 맨 앞에 것만 바꾸는게 아니라 전부 다 바꾼다는 의미입니다.
