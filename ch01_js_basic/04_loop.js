...
/*
  ...
  두번째 Array의 element인 object의 마지막 property의 value 값 다음에도 ','가 있지만 오류가 발생하지 않는다는 점

  입니다. 

  이거 추후에 property를 추가하거나 혹은 element를 추가하거나 할 때 전에 말한것처럼 쉼표 치고 엔터 치고 다시 {} 혹은 property 추가를 하기 귀찮으니까
  맨 마지막 element / property 여부와 관계 없이 쉼표 찍어주는게 매너가 된게 아에 관행처럼 굳어진겁니다. 그래서 오류로 처리하지도 않습니다.
...
let result = productList[0].price + productList[2].price;
console.log(productList[0].name+ '의 가격과 ' + productList[2].name + '의 가격은 각각 ' + productList[0].price + '원과 ' + productList[2].price+ '원으로, 그 합은 ' + result + '원 입니다.');
...
/*
  ...
  그럼 마찬가지로 일반 객체의 각 property를 뽑아내거나 추출해서 데이터를 가공하는 방식이 불가능함을 의미합니다.
*/
...
// 그래서 해당 key에 할당된 value를 추출하고 싶다면 113번 라인과 같은 방식으로 코드를 작성해야합니다.
...
// 그래서 굳이 # 1, 2로 두가지 방식을 통해서 객체의 value를 조회할 수 있었습니다.
...
// 객체의 property의 key를 추출할 수 있으니까, 마치 향상된 for문처럼 여겨져서
...
// 3. for-of문
// for-in문의 경우는 객체 한정으로 생각하시는 편이 낫습니다.
