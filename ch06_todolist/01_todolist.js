...
// 7번 라인의 해석은 localStorage.getItem('todos') 에 데이터가 있으면 true이기 때문에 JSON으로 바꿔주고,
...
function renderTodos() {
  ...
  // 여러분들은 todoList(HTML요소) =/= todos(JSON)
  todoList.innerHTML ='';
  ...
  todos.forEach((todo, index) => {
    ...
    // 보통은 첫 번재가 반복문 돌 때의 element의 이름을 선언합니다.
    // 두번째가 index 관련이라고 생각하시면 됩니다.

    // 각 todo는 JS 객체에 해당하는거고, 이걸 페이지 상에서 보여주기 위해서는 ul 태그의 자식인 li 태그가 필요함

    // 그럼 얘도 지역변수
    const li = document.createElement('li');
    // li 태그만 만들었지 클래스 이름 안정했으니까
    li.className = 'todo-item';
    // Java의 객체에 field값 대입하는 방법과 동일
    if(todo.completed) {    
      // localStorage의 'todos' key의 내부를 확인했을 때 배열로 저장되어 있고, 거기 내부에 JS객체가 있는데, 그 객체가 현재 todo라고 이름 붙여져 있습니다. 그 todo 들은 text라는 key, completed라는 key를 가지고 있었습니다.
      // 그 completed는 자료형이 boolean이었고, 걔가 true / false인지에 따라서 35번 라인의 조건문 실행 여부가 결정될겁니다.
      // .css 파일에서 completed 속성 부분을 적용해주기 위해서 쓴 코드라인이 됩니다.
      // completed가 true라면 할 일이 완료되었음을 표시해주기 위해서 css 상 차이를 만들었는데, 그걸 적용하기 위한거죠.
      li.classList.add('completed');  // completed의 css 속성을 추가해주겠다.
    }
    // 35번 라인부터 40번 라인까지의 해석은 JS 객체 상의 key가 completed인 애의 value가 true라면 li 태그에 completed라는 속성을 추가해 주는데, 이게 추가되면 .css에서 정의된 추가 스타일을 적용하기 위한것입니다. (취소선, 배경색 등)

    // li 태그의 자식 요소로 input checkbox / span / button 으로 이루어져 있기 때문에 각각을 또 요소를 만들 필요가 있겠네요.
    ...
    // checked 자체는 true false 와 상관없이 계속 유지가 됩니다.
    // input의 type이 checkbox라면 checked하고 =""가 없는 애가 있었습니다. <input type="checkbox" checked> 형태였습니다.
    ...
    span.textContent = todo.text;   // 내용이 있는거니까 배열내의 element인 JS 객체의 text라는 키의 value를 span태그의 내부에 집어넣어서 페이지 상에 출력

    // 삭제버튼 - button 태그
    const deleteBtn = document.createElement('button');
    deleteBtn.className = 'delete-btn';
    deleteBtn.innerHTML = '&times'; // x 모양 기호

    // 요소들을 li 변수에 추가해야 합니다. 왜? li 내부에 체크박스/span/버튼
    li.append(checkbox);
    li.append(span);
    li.append(deleteBtn);
...
function saveTodos() {
  ...
  localStorage.setItem('temp', '안녕');   // 7번 라인 때문에 항상 localStorage의 value가 배열이라고 오해살까봐 추가한 코드 -> 여기는 value가 string
}  
...
function addTodo() {
  ...
  renderTodos();  // 추가 버튼 누르고 나서 다시 (갱신된) 리스트 가지고 와야하니까 renderTodos 함수 호출
...
// 새로고침했을 때 renderTodos()가 호출 되어야함.
